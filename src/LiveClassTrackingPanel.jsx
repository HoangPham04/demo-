...
const DEMO_STUDENTS = [
  {
    ...
    userName: "Student Demo 1",
    email: "student.demo1@test.com",
  },
  {
    userId: "student-demo-002",
    userName: "Student Demo 2",
    email: "student.demo2@test.com",
  },
  {
    userId: "student-demo-003",
    userName: "Student Demo 3",
    email: "student.demo3@test.com",
  },
  {
    userId: "student-demo-004",
    userName: "Student Demo 4",
    email: "student.demo4@test.com",
  },
  {
    userId: "student-demo-005",
    userName: "Student Demo 5",
    email: "student.demo5@test.com",
  },
];
...
export default function LiveClassTrackingPanel({
  ...
  const [selectedStudentId, setSelectedStudentId] = useState(
    DEMO_STUDENTS[0].userId
  );

  const selectedStudent =
    DEMO_STUDENTS.find((student) => student.userId === selectedStudentId) ||
    DEMO_STUDENTS[0];
  ...
  const payload = useMemo(
    ...
    [selectedStudent, groupCode, meetLink]
  );
  ...
  const handleLogin = () => {
    runAction(
      () => liveClassLogin(payload),
      `${selectedStudent.userName} login tracked.`
    );
  };

  const handleLogout = () => {
    runAction(
      () => liveClassLogout(payload),
      `${selectedStudent.userName} logout tracked.`
    );
  };
  ...
  const handleDeviceCheck = async () => {
    ...
    runAction(
      ...
      `${selectedStudent.userName} device checked. Camera: ${
        cameraReady ? "OK" : "Failed"
      }, Mic: ${microphoneReady ? "OK" : "Failed"}.`
    );
  };
  ...
  const handleScreenCheck = async () => {
    ...
    runAction(
      ...
      `${selectedStudent.userName} screen share checked. Result: ${
        screenReady ? "OK" : "Failed"
      }.`
    );
  };
  ...
  const handleJoinMeet = async () => {
    await runAction(
      ...
      `${selectedStudent.userName} Join Google Meet tracked.`
    );
  ...
  return (
    <div className="live-tracking-panel">
      <style>{`
        ...
        .tracking-student-select {
          min-width: 260px;
        }

        .tracking-student-select label {
          display: block;
          margin-bottom: 7px;
          color: #344054;
          font-size: 13px;
        ...
        .tracking-student-select select {
          width: 100%;
        ...
        .tracking-student-select select:focus {
          border-color: #2563eb;
      ...
      <div className="tracking-header">
        <div>
          ...
          <p className="tracking-description">
            Chọn từng học viên demo để test login/logout, camera, microphone,
            screen share và Join Google Meet.
          </p>
        </div>

        <div className="tracking-student-select">
      ...
      <div className="selected-student-card">
        Testing as: <strong>{selectedStudent.userName}</strong> ·{" "}
