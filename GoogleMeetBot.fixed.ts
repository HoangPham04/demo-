...
export class GoogleMeetBot extends MeetBotBase {
  ...
  async join({ url, name, bearerToken, teamId, timezone, userId, eventId, botId, uploader }: JoinParams): Promise<void> {
    ...
    const safePatchBotStatus = async () => {
      try {
        await patchBotStatus({ botId, eventId, provider: 'google', status: _state, token: bearerToken }, this._logger);
      } catch (patchError) {
        this._logger.warn('Unable to patch bot status. This is expected when running standalone without ScreenApp backend.', { patchError });
      }
    };
  ...
  private async joinMeeting({ url, name, teamId, userId, eventId, botId, pushState, uploader }: JoinParams & { pushState(state: BotStatus): void }): Promise<void> {
    ...
    this._logger.info('Waiting for Google Meet name input / join screen...');

    await this.page.screenshot({
      path: '/tmp/google-meet-before-name-input.png',
      fullPage: true,
    });

    await retryActionWithWait(
      'Waiting for any visible Google Meet name input or join screen',
      async () => {
        ...
        this._logger.info('Google Meet page state before name input', pageState);

        const signInDetected =
          pageState.url.includes('accounts.google.com') ||
          pageState.bodyText.includes('Sign in') ||
          pageState.bodyText.includes('Use your Google Account');

        if (signInDetected) {
          await this.page.screenshot({
            path: '/tmp/google-meet-sign-in-detected.png',
            fullPage: true,
          });
          throw new UnsupportedMeetingError('Meeting requires sign in', 'SIGN_IN_PAGE');
        }

        const possibleNameInputs = this.page.locator(
          [
        ...
        const inputCount = await possibleNameInputs.count();

        if (inputCount > 0) {
          const input = possibleNameInputs.first();
          await input.waitFor({ state: 'visible', timeout: 10000 });
          return;
        }

        const possibleJoinButtons = this.page.locator('button', {
          hasText: /Ask to join|Join now|Join anyway|Continue/i,
        });

        const joinButtonCount = await possibleJoinButtons.count();

        if (joinButtonCount > 0) {
          return;
        }

        throw new Error('No visible name input or join button found on Google Meet page.');
      },
      this._logger,
      3,
      15000,
      async () => {
        await this.page.screenshot({
          path: '/tmp/google-meet-input-field-wait-failed.png',
          fullPage: true,
        });
    ...
    this._logger.info('Trying to fill the visible name input if present...');
    ...
    if ((await possibleNameInputs.count()) > 0) {
      const input = possibleNameInputs.first();
      await input.fill(name ? name : 'ScreenApp Notetaker');
      this._logger.info('Filled Google Meet display name.');
    } else {
      this._logger.info('Name input is missing. Bot may already be signed in or Google Meet skipped name entry.');
    }

    this._logger.info('Waiting for 3 seconds before join action...');
    await this.page.waitForTimeout(3000);

    await retryActionWithWait(
      'Clicking Google Meet join button',
      async () => {
        const possibleTexts = [
          'Ask to join',
          'Join now',
        ...
        for (const text of possibleTexts) {
          try {
            ...
            if ((await button.count()) > 0 && (await button.isVisible())) {
              ...
              buttonClicked = true;
              this._logger.info(`Successfully clicked Google Meet button using text "${text}".`);
              break;
            }
          } catch (err) {
            this._logger.warn(`Unable to click Google Meet button using text "${text}".`, { err });
          }
        }

        if (!buttonClicked) {
          await this.page.screenshot({
            path: '/tmp/google-meet-join-button-failed.png',
            fullPage: true,
          });

          throw new Error('Unable to complete the Google Meet join action.');
        }
      },
      this._logger,
      3,
      ...
      async () => {
        await this.page.screenshot({
          path: '/tmp/google-meet-ask-to-join-button-click-failed.png',
          fullPage: true,
        });
